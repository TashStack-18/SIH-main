...
export function BharatMap({
  ...
  const effectiveIsDark = propIsDark !== undefined ? propIsDark : isThemeDark;

  // Default mode is 3D Map
  ...
  useEffect(() => {
    ...
    import('mapbox-gl')
      .then((mapboxglModule) => {
        ...
        try {
          // Initialize with default bright, vibrant daylight outdoors style
  ...
  useEffect(() => {
    ...
    validStops.forEach((stop) => {
      ...
      // Clean, bright default pill label below pin
    ...
    for (let tx = minX; tx <= maxX; tx++) {
      for (let ty = minY; ty <= maxY; ty++) {
        ...
        if (mapMode === 'SATELLITE') {
          url = `https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${slippyZoom}/${ty}/${wrappedTx}`;
        } else if (mapMode === '2D') {
          url = `https://tile.openstreetmap.org/${slippyZoom}/${wrappedTx}/${ty}.png`;
        } else {
          // Default 3D daylight street basemap (Esri World Street Map - zero watermark, zero API key required, global high-res cartography)
          url = `https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/${slippyZoom}/${ty}/${wrappedTx}`;
        }
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: 'hidden',
        background: effectiveIsDark ? '#1C1E2D' : '#FAF7F2',
        border: effectiveIsDark ? '1px solid rgba(255, 255, 255, 0.15)' : '1px solid rgba(200, 142, 68, 0.35)',
        boxShadow: effectiveIsDark ? '0 12px 36px rgba(0, 0, 0, 0.4)' : '0 12px 36px rgba(45, 27, 20, 0.1)',
      }}
    >
      ...
      {showControls && (
        ...
        >
          ...
          <div
            style={{
              display: 'flex',
              background: effectiveIsDark ? 'rgba(28, 30, 45, 0.94)' : 'rgba(255, 255, 255, 0.94)',
              backdropFilter: 'blur(12px)',
              border: effectiveIsDark ? '1px solid rgba(255, 255, 255, 0.18)' : '1px solid rgba(200, 142, 68, 0.3)',
              borderRadius: '30px',
              padding: '3px',
              gap: '2px',
              pointerEvents: 'auto',
              boxShadow: effectiveIsDark ? '0 6px 20px rgba(0, 0, 0, 0.35)' : '0 6px 20px rgba(45, 27, 20, 0.12)',
            }}
          >
            {[
              ...
              <button
                ...
                style={{
                  ...
                  color: mapMode === mode.id ? '#FFFFFF' : effectiveIsDark ? '#C9CCD6' : '#4A3C31',
          ...
          <div style={{ display: 'flex', gap: '6px', pointerEvents: 'auto' }}>
            <button
              ...
              style={{
                ...
                backdropFilter: 'blur(12px)',
                border: effectiveIsDark ? '1px solid rgba(255, 255, 255, 0.18)' : '1px solid rgba(200, 142, 68, 0.3)',
                color: effectiveIsDark ? '#FFFFFF' : '#2D1B14',
                padding: '5px 12px',
                borderRadius: '20px',
                fontSize: '0.74rem',
                fontWeight: 700,
