...
export default function GoogleMapView() {
  ...
  useEffect(() => {
    const checkDark = () => {
      const dark =
        ...
        localStorage.getItem('bsy_theme') === 'dark';
      setIsDark(dark);
    };
    checkDark();
  ...
  return (
    <div style={{ minHeight: '100vh', background: isDark ? '#131B2E' : '#FAF7F2', color: isDark ? '#F1F5F9' : '#2D1B14', paddingBottom: '70px', transition: 'background-color 0.25s ease, color 0.25s ease' }}>
      {/* Top Heritage Header */}
      <div
        style={{
          background: isDark ? 'linear-gradient(180deg, #1C1E2D 0%, #131B2E 100%)' : 'linear-gradient(180deg, #FFFFFF 0%, #FAF7F2 100%)',
          borderBottom: isDark ? '1px solid rgba(255, 255, 255, 0.08)' : '1px solid rgba(200, 142, 68, 0.25)',
          padding: '28px 20px 20px',
          boxShadow: isDark ? '0 4px 20px rgba(0, 0, 0, 0.25)' : '0 4px 16px rgba(45, 27, 20, 0.04)',
        }}
      >
        <div style={{ maxWidth: '1380px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px' }}>
            <div>
              ...
              <h1 style={{ fontSize: '1.85rem', fontWeight: 800, margin: '6px 0 2px', color: isDark ? '#FFFFFF' : '#2D1B14', letterSpacing: '-0.02em' }}>
                Interactive Union Territory Map Explorer
              </h1>
              <p style={{ fontSize: '0.9rem', color: isDark ? '#94A3B8' : '#78685C', margin: 0 }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    padding: '8px 18px',
                    borderRadius: '30px',
                    border: `1.5px solid ${isSelected ? clr.border : isDark ? 'rgba(255, 255, 255, 0.15)' : 'rgba(200, 142, 68, 0.25)'}`,
                    background: isSelected ? clr.active : isDark ? '#1E2235' : '#FFFFFF',
                    color: isSelected ? '#FFFFFF' : isDark ? '#E2E8F0' : '#4A3C31',
                    fontSize: '0.82rem',
                    fontWeight: isSelected ? 800 : 600,
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
                    boxShadow: isSelected ? `0 4px 14px ${clr.shadow}` : isDark ? '0 2px 6px rgba(0,0,0,0.2)' : '0 2px 6px rgba(45, 27, 20, 0.04)',
                  }}
                >
                  {ut.shortName}
                </button>
      ...
      <div style={{ maxWidth: '1380px', margin: '24px auto 0', padding: '0 20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button
              ...
              style={{
                padding: '6px 16px',
                borderRadius: '20px',
                border: `1.5px solid ${activeCategory === 'ALL' ? '#0284c7' : isDark ? 'rgba(255, 255, 255, 0.15)' : 'rgba(200, 142, 68, 0.25)'}`,
                fontSize: '0.8rem',
                fontWeight: 700,
                cursor: 'pointer',
                background: activeCategory === 'ALL' ? '#0284c7' : isDark ? '#1E2235' : '#FFFFFF',
                color: activeCategory === 'ALL' ? '#FFFFFF' : isDark ? '#E2E8F0' : '#4A3C31',
                boxShadow: activeCategory === 'ALL' ? '0 2px 8px rgba(2, 132, 199, 0.3)' : isDark ? '0 1px 4px rgba(0,0,0,0.2)' : '0 1px 4px rgba(0,0,0,0.04)',
              }}
            >
            ...
            <button
              ...
              style={{
                padding: '6px 16px',
                borderRadius: '20px',
                border: `1.5px solid ${activeCategory === 'ATTRACTIONS' ? '#C88E44' : isDark ? 'rgba(255, 255, 255, 0.15)' : 'rgba(200, 142, 68, 0.25)'}`,
                fontSize: '0.8rem',
                fontWeight: 700,
                cursor: 'pointer',
                background: activeCategory === 'ATTRACTIONS' ? '#C88E44' : isDark ? '#1E2235' : '#FFFFFF',
                color: activeCategory === 'ATTRACTIONS' ? '#FFFFFF' : isDark ? '#E2E8F0' : '#4A3C31',
                boxShadow: activeCategory === 'ATTRACTIONS' ? '0 2px 8px rgba(200, 142, 68, 0.35)' : isDark ? '0 1px 4px rgba(0,0,0,0.2)' : '0 1px 4px rgba(0,0,0,0.04)',
              }}
            >
            ...
            <button
              ...
              style={{
                padding: '6px 16px',
                borderRadius: '20px',
                border: `1.5px solid ${activeCategory === 'EMERGENCY' ? '#059669' : isDark ? 'rgba(255, 255, 255, 0.15)' : 'rgba(200, 142, 68, 0.25)'}`,
                fontSize: '0.8rem',
                fontWeight: 700,
                cursor: 'pointer',
                background: activeCategory === 'EMERGENCY' ? '#059669' : isDark ? '#1E2235' : '#FFFFFF',
                color: activeCategory === 'EMERGENCY' ? '#FFFFFF' : isDark ? '#E2E8F0' : '#4A3C31',
                boxShadow: activeCategory === 'EMERGENCY' ? '0 2px 8px rgba(5, 150, 105, 0.3)' : isDark ? '0 1px 4px rgba(0,0,0,0.2)' : '0 1px 4px rgba(0,0,0,0.04)',
              }}
            >
          ...
          <div style={{ fontSize: '0.85rem', color: isDark ? '#94A3B8' : '#78685C' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: '24px', marginTop: '24px' }}>
          ...
          {selectedPlace && (
            <div
              style={{
                background: isDark ? '#1C1E2D' : '#FFFFFF',
                border: isDark ? '1px solid rgba(255, 255, 255, 0.12)' : '1.5px solid rgba(200, 142, 68, 0.35)',
                borderRadius: '16px',
                padding: '24px',
                boxShadow: isDark ? '0 10px 30px rgba(0, 0, 0, 0.35)' : '0 10px 30px rgba(45, 27, 20, 0.08)',
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '0.78rem', color: isDark ? '#94A3B8' : '#78685C', fontWeight: 600 }}>
                      {selectedPlace.territoryName || selectedUT}
                    </span>
                  </div>
                  <h3 style={{ fontSize: '1.45rem', fontWeight: 800, color: isDark ? '#FFFFFF' : '#2D1B14', margin: '6px 0 4px' }}>
                    {selectedPlace.name}
                  </h3>
                ...
                {liveWeather && (
                  <div
                    style={{
                      background: isDark ? '#131B2E' : '#FAF7F2',
                      border: isDark ? '1px solid rgba(255, 255, 255, 0.12)' : '1px solid rgba(200, 142, 68, 0.25)',
                      borderRadius: '12px',
                      padding: '8px 14px',
                      textAlign: 'right',
                      minWidth: '130px',
                      boxShadow: isDark ? '0 2px 6px rgba(0, 0, 0, 0.2)' : '0 2px 6px rgba(45, 27, 20, 0.04)',
                    }}
                  >
                    <div style={{ fontSize: '1.25rem', fontWeight: 800, color: isDark ? '#FFFFFF' : '#2D1B14' }}>
                      {liveWeather.tempC}°C
                    </div>
              ...
              <p style={{ fontSize: '0.9rem', color: isDark ? '#CBD5E1' : '#4A3C31', lineHeight: '1.6', margin: '14px 0 16px' }}>
              ...
              {/* Action Buttons */}
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', paddingTop: '12px', borderTop: isDark ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid rgba(200, 142, 68, 0.15)' }}>
                {'slug' in selectedPlace && selectedPlace.slug && (
                  <>
                    ...
                    <Link
                      ...
                      style={{
                        background: isDark ? '#131B2E' : '#FAF7F2',
                        color: isDark ? '#FFFFFF' : '#2D1B14',
                        padding: '9px 18px',
                        borderRadius: '8px',
                        fontSize: '0.84rem',
                        fontWeight: 700,
                        textDecoration: 'none',
                        border: isDark ? '1px solid rgba(255, 255, 255, 0.2)' : '1px solid rgba(200, 142, 68, 0.3)',
                      }}
                    >
                      Explore Destination ↗
                    </Link>
          ...
          {/* Territory Route Navigation Circuit Card */}
          <div
            style={{
              background: isDark ? '#1C1E2D' : '#FFFFFF',
              border: isDark ? '1px solid rgba(255, 255, 255, 0.12)' : '1.5px solid rgba(200, 142, 68, 0.35)',
          ...
          >
            <div>
              ...
              <h4 style={{ fontSize: '1.25rem', fontWeight: 800, color: isDark ? '#FFFFFF' : '#2D1B14', margin: '6px 0 4px' }}>
                {UT_LIST.find((u) => u.id === selectedUT)?.name} Verified Heritage & Scenic Circuit
              </h4>
              <p style={{ fontSize: '0.85rem', color: isDark ? '#94A3B8' : '#78685C', margin: 0 }}>
              ...
              {routeResult && (
                <div style={{ display: 'flex', gap: '16px', marginTop: '18px' }}>
                  <div style={{ background: isDark ? '#131B2E' : '#FAF7F2', padding: '10px 16px', borderRadius: '10px', border: isDark ? '1px solid rgba(255, 255, 255, 0.12)' : '1px solid rgba(200, 142, 68, 0.25)' }}>
                    <div style={{ fontSize: '0.72rem', color: isDark ? '#94A3B8' : '#78685C' }}>Total Route Distance</div>
                    <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#0284c7' }}>
                      ~{routeResult.totalDistanceKm} km
                    </div>
                  </div>
                  <div style={{ background: isDark ? '#131B2E' : '#FAF7F2', padding: '10px 16px', borderRadius: '10px', border: isDark ? '1px solid rgba(255, 255, 255, 0.12)' : '1px solid rgba(200, 142, 68, 0.25)' }}>
                    <div style={{ fontSize: '0.72rem', color: isDark ? '#94A3B8' : '#78685C' }}>Est. Transit Drive Time</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '22px', paddingTop: '14px', borderTop: isDark ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid rgba(200, 142, 68, 0.15)' }}>
              <div style={{ fontSize: '0.78rem', color: isDark ? '#94A3B8' : '#78685C' }}>
