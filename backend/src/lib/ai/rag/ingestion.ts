  ...
  public initialize(): KnowledgeChunk[] {
    ...
    for (const dest of VERIFIED_DESTINATIONS) {
      ...
      allChunks.push({
        ...
        content: [
          `Destination: ${dest.name} (${dest.territoryName})`,
          dest.tagline ? `Tagline: ${dest.tagline}.` : '',
          `Type: ${dest.type}.`,
          dest.coordinates?.lat ? `Coordinates: ${dest.coordinates.lat}, ${dest.coordinates.lng}.` : '',
          `Overview: ${dest.overview || dest.shortDescription}`,
          dest.highlights && dest.highlights.length > 0 ? `Highlights: ${dest.highlights.join('; ')}.` : '',
          dest.thingsToDo && dest.thingsToDo.length > 0 ? `Things to do: ${dest.thingsToDo.map((t) => t.title + ': ' + t.desc).join('; ')}.` : '',
        ].filter(Boolean).join('\n'),
        territorySlug,
      ...
      allChunks.push({
        ...
        content: [
          ...
          `Nearest Emergency Medical Facility: ${dest.safety?.emergencyFacility || 'Dial 112 for statutory emergency response'}.`,
          `Weather advisory: Best time is ${dest.weather?.bestTime || 'Seasonal'}. Summer temp: ${dest.weather?.tempSummer || 'Moderate'}; Winter temp: ${dest.weather?.tempWinter || 'Cool'}.`,
        ].filter(Boolean).join('\n'),
        territorySlug,
        destinationId: dest.id,
        category: 'PERMIT',
      ...
      if (dest.food || dest.culture) {
        allChunks.push({
          ...
          content: [
            `Destination: ${dest.name} (${dest.territoryName})`,
            dest.food?.overview ? `Culinary overview: ${dest.food.overview}.` : '',
            dest.food?.dishes && dest.food.dishes.length > 0 ? `Specialty dishes: ${dest.food.dishes.join(', ')}.` : '',
            dest.culture?.languages && dest.culture.languages.length > 0 ? `Languages: ${dest.culture.languages.join(', ')}.` : '',
            dest.culture?.traditions ? `Traditions: ${dest.culture.traditions}.` : '',
            dest.culture?.etiquette ? `Etiquette: ${dest.culture.etiquette}.` : '',
          ].filter(Boolean).join('\n'),
          territorySlug,
          destinationId: dest.id,
          category: 'CULTURE',
    ...
    for (const ut of VERIFIED_TERRITORIES) {
      ...
      allChunks.push({
        ...
        content: [
          `Union Territory: ${ut.name} (Code: ${ut.code}, Capital: ${ut.capital})`,
          ut.tagline ? `Tagline: ${ut.tagline}.` : '',
          expText ? `Signature Experiences: ${expText}.` : '',
          popText ? `Popular destinations: ${popText}.` : '',
          advText ? `Official Advisories & Entry Rules: ${advText}.` : '',
          `Official Portal: ${ut.officialPortal}`,
          `ePermit Portal: ${ut.slug === 'lakshadweep' ? 'https://epermit.utl.gov.in' : 'Official UT Permit Desk'}`,
          `Emergency numbers: Unified 112, Tourist Helpline 1363`,
        ].filter(Boolean).join('\n'),
